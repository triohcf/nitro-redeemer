...
def activate_license():
    print(f"[INPUT] Enter your license key:", end=" ")
    license_key = input()
    LexActivator.SetLicenseKey(license_key)
    status = LexActivator.ActivateLicense()
    if LexStatusCodes.LA_OK == status:
        print(f"[SUCCESS] License activated successfully.")
        return True
    else:
        print(f"[FAIL] License activation failed")
        return False
...
def check_license():
    try:
        ...
        LexActivator.SetProductId("", PermissionFlags.LA_USER)
        status = LexActivator.IsLicenseGenuine()
        if LexStatusCodes.LA_OK == status:
            print(f"[SUCCESS] License is activated.")
            return True
        elif LexStatusCodes.LA_EXPIRED == status:
            print(f"[EXPIRED] License has expired.")
            if activate_license():
                return True
            else:
                return False
        elif LexStatusCodes.LA_SUSPENDED == status:
            print(f"[SUSPENDED] License has been suspended.")
            if activate_license():
                return True
            else:
                return False
            ...
            try:
                ...
                while i < 25:
                    ...
                    if "svgBorder-2bdygG" in emulator.page_source:
                        print(f"{Fore.LIGHTGREEN_EX} [SUCCESS] Nitro successfully activated for token {str(token)}")
                        emulator.close()
                        return "successful"
...
def logo():
    ...
    os.system("title " + 'Aidens Services | Nitro Redeemer')
...
if __name__ == '__main__':
    ...
    if not tokens or not links or not ccs:
        print(f"{Fore.LIGHTRED_EX} [!!!] Fill in the required tokens/links/ccs in the respective files.")
        print(f"{Fore.LIGHTBLUE_EX} [>>>] Press enter to close.", end="")
        input()
        exit(0)

    if len(links) < len(tokens):
        print(
            f"{Fore.LIGHTBLUE_EX} [>>>] Activating {len(links)} links out of {len(tokens)} tokens.")
        tokens = tokens[0:len(links)]

    print(f"{Fore.LIGHTBLUE_EX} [>>>] Uses per VCC:", end=" ")
    card_uses = int(input())

    print(f"{Fore.LIGHTGREEN_EX} [>>>] Redeeming on {len(tokens)} accounts.")
    successful = 0
    current_link = 0
    current_card = 0
    current_card_uses = 0
    ...
    if successful == 0:
        print(f"{Fore.RED} [ERROR] No accounts have been activated.")
    else:
        print(f"{Fore.LIGHTGREEN_EX} [>>>] Successfully redeemed nitro on {successful} accounts!")
    print(f"{Fore.LIGHTBLUE_EX} [>>>] Press enter to close.", end="")
    input()
    exit(0)
